// Test script for production MCP server
async function testMCPServer() {
  ...
  // Test 4: Semantic search (if API key is configured)
  console.log('4. Testing semantic search for "groundwater flow visualization":');
  try {
    const semanticResponse = await fetch(baseUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        jsonrpc: '2.0',
        ...
        params: {
          name: 'mfai_search',
          arguments: {
            query: 'groundwater flow visualization',
    ...
    console.log('Response:', JSON.stringify(semanticResult, null, 2).substring(0, 500) + '...');
  } catch (error) {
    console.error('Error:', error.message);
  }
  
  console.log('\n---');
  console.log('Production URL for MCP clients:');
  console.log('https://mfai-repository-navigator.little-grass-273a.workers.dev/mcp');
}
